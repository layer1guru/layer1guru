<script setup lang="ts">
useHead({
    title: `Blank — Nexa Studio`,
    meta: [
        { name: 'description', content: `Nexa Studio makes building your next BIG idea effortless.` }
    ],
})

/* Initialize stores. */
import { useSystemStore } from '@/stores/system'
const System = useSystemStore()

const emit = defineEmits(['openPanel'])

const gurus = ref(null)
const subnets = ref(null)

const isShowingOptionsMenu = ref(null)
const isShowingGurus = ref(null)
const isShowingSubnets = ref(null)

const openPanel = () => {
    emit('openPanel')
}

const showGurus = () => {
    isShowingGurus.value = true
    isShowingSubnets.value = false
}

const showSubnets = () => {
    isShowingGurus.value = false
    isShowingSubnets.value = true
}

const init = () => {
    isShowingOptionsMenu.value = false

    /* Initialize gurus. */
    gurus.value = []
    isShowingGurus.value = true

    /* Initialize subnets. */
    subnets.value = []
    isShowingSubnets.value = false

    /* Load Gurus. */
    gurus.value.push({
        id: '02264e27-4d37-45b5-aac1-8a064315a26c',
        title: 'Fuji Tester',
    })

    gurus.value.push({
        id: 'b893b1dd-a7d6-4cb5-85b0-9034bccdd3ea',
        title: 'Production (Mainnet)',
    })

    gurus.value.push({
        id: 'b9ba5062-5519-4376-bdfe-d29569010392',
        title: 'Dev Runner',
    })

    /* Load Subnets. */
    subnets.value.push({
        id: 'f02d52e7-2318-46b7-a7b1-719a7e388195',
        title: 'Nexaverse',
    })

    subnets.value.push({
        id: '4b10eedd-fb27-4c63-8d6a-0bfddf7c65f0',
        title: 'Incognito',
    })

    subnets.value.push({
        id: '1ad9a6ab-f8aa-4e2b-8d6e-3d4834bff569',
        title: 'Real Money Gaming',
    })

}

onMounted(() => {
    init()
})

// onBeforeUnmount(() => {
//     console.log('Before Unmount!')
//     // Now is the time to perform all cleanup operations.
// })
</script>

<template>
    <div class="fixed top-0 left-0 h-full w-1/2 bg-white" aria-hidden="true"></div>
    <div class="fixed top-0 right-0 h-full w-1/2 bg-gray-50" aria-hidden="true"></div>

    <main class="relative flex min-h-full flex-col">
        <Header
            @openPanel="openPanel"
        />

        <div class="mx-auto w-full max-w-7xl flex-grow lg:flex xl:px-8">
            <div class="min-w-0 flex-1 bg-white xl:flex">
                <SideBar
                    :gurus="gurus"
                    @openPanel="openPanel"
                    @showGurus="showGurus"
                    @showSubnets="showSubnets"
                />

                <MyGurus v-if="isShowingGurus" :gurus="gurus" />
                <MySubnets v-if="isShowingSubnets" :subnets="subnets" />
            </div>

            <ActivityFeed />
        </div>
    </main>
</template>
